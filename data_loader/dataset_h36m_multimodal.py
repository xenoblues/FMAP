    ...
    def process_data(self):
        ...
        if self.multimodal_path is None:
            self.data_multimodal = \
                np.load('../data/data_multi_modal/t_his25_1_thre0.500_t_pred100_thre0.100_filtered_dlow.npz',
                        allow_pickle=True, mmap_mode='r')['data_multimodal'].item()
            data_candi = \
                np.load('../data/data_multi_modal/data_candi_t_his25_t_pred100_skiprate20.npz', allow_pickle=True,
                        mmap_mode='r')['data_candidate.npy']
        else:
            ...
            for act in data_s.keys():
                ...
                for i in range(0, seq_len - self.t_total, step):
                    ...
                    if n_modality > 0:
                        ...
                        # future distance
                        dist_pred = np.mean(np.linalg.norm(x0[:, self.t_his:, 1:] -
                                                           candi_tmp[idx_his, self.t_his:self.t_total, 1:], axis=3), axis=(1, 2))

                        idx_pred = np.where(dist_pred >= thre_pred)[0]
                        # idxs = np.intersect1d(idx_his, idx_pred)
                        traj_multi = candi_tmp[idx_his[idx_pred], :self.t_total]
                        if len(traj_multi) > 0:
...
if __name__ == '__main__':
    ...
    actions = {'WalkDog'}
    t0 = time.time()
    dataset = DatasetH36M_multi('train')
    print(time.time() - t0)
